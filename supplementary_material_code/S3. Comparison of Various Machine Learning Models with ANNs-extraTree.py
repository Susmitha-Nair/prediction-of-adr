...
if __name__ == "__main__":
    base_path = '/sas/vidhya/susmitha/ADR_Prediction/final_results/extraTree/pca/subset_1/' # 'C:\\Users\\Aditya R\\Downloads\\'
    ...
    file = pd.read_csv(base_path + pca_path)
    file2 = pd.read_csv(base_path + adr_path)
    file2 = file2.rename(columns={file2.columns[0]: 'name'})

    validation_data_pca = pd.read_csv(base_path + pred_subset_path)
    validation_data_pca = validation_data_pca.rename(columns={validation_data_pca.columns[0]: 'name'})

    validation_data_adr = pd.read_csv(base_path + adr_subset_path)
    ...
    chunks = list(divide_chunks(adrs_all, 5))
